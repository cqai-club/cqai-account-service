...
- [ ] 确认服务器已安装 Docker，部署用户可以运行 Docker，并拥有 `systemctl disable --now/enable --now cqai-account-service` 的非交互最小 sudo 权限；Actions deploy job 使用工作流自带的 `GITHUB_TOKEN` 读取 GHCR。
